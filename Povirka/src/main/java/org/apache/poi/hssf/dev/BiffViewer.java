...
public final class BiffViewer {
    ...
    static void hexDumpAligned(Writer w, byte[] data, int dumpLen, int globalOffset, boolean zeroAlignEachRecord) {
        ...
        if (zeroAlignEachRecord) {
            int endLineAddr3 = (globalEnd - endDelta) - (globalStart - startDelta);
            endLineAddr = endLineAddr3;
            endLineAddr2 = 0;
        } else {
            int startLineAddr = globalStart - startDelta;
            endLineAddr = globalEnd - endDelta;
            endLineAddr2 = startLineAddr;
        }
        ...
        while (true) {
            ...
            int startLineAddr2 = endLineAddr2;
            hexDumpLine(w, data, lineAddr, lineDataOffset, 0, 16);
            lineAddr2 = lineAddr;
            endLineAddr2 = startLineAddr2;
        }
        if (endDelta != 0) {
    ...
    private static void hexDumpLine(Writer w, byte[] data, int lineStartAddress, int lineDataOffset, int startDelta, int endDelta) {
        char[] cArr = COLUMN_SEPARATOR;
        char[] buf = new char[((((cArr.length * 2) + 8) + 48) - 1) + 16 + NEW_LINE_CHARS.length];
        if (startDelta >= endDelta) {
            throw new IllegalArgumentException("Bad start/end delta");
        }
        try {
            writeHex(buf, 0, lineStartAddress, 8);
            int idx = arraycopy(cArr, buf, 0 + 8);
            int idx2 = idx;
            for (int i = 0; i < 16; i++) {
                if (i > 0) {
                    int idx3 = idx2 + 1;
                ...
                if (i >= startDelta && i < endDelta) {
                    writeHex(buf, idx2, data[lineDataOffset + i], 2);
                } else {
                    buf[idx2] = ' ';
                    buf[idx2 + 1] = ' ';
                }
            ...
            int i2 = 0;
            int idx4 = arraycopy(COLUMN_SEPARATOR, buf, idx2);
            while (i2 < 16) {
                char ch = ' ';
                if (i2 >= startDelta && i2 < endDelta) {
                    ch = getPrintableChar(data[lineDataOffset + i2]);
                }
                int idx5 = idx4 + 1;
                try {
                    buf[idx4] = ch;
            ...
            w.write(buf, 0, arraycopy(NEW_LINE_CHARS, buf, idx4));
