...
public abstract class CFRuleBase extends StandardRecord implements Cloneable {
    ...
    protected int readFormatOptions(RecordInputStream in) {
        ...
        if (containsFontFormattingBlock()) {
            FontFormatting fontFormatting = new FontFormatting(in);
            this._fontFormatting = fontFormatting;
            len = 6 + fontFormatting.getDataLength();
        }
        if (containsBorderFormattingBlock()) {
            BorderFormatting borderFormatting = new BorderFormatting(in);
            this._borderFormatting = borderFormatting;
            len += borderFormatting.getDataLength();
        }
        if (containsPatternFormattingBlock()) {
            PatternFormatting patternFormatting = new PatternFormatting(in);
            this._patternFormatting = patternFormatting;
            return len + patternFormatting.getDataLength();
        }
        return len;
    }
