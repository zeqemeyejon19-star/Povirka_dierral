...
public class CellReference {
    ...
    public CellReference(String cellRef) {
        ...
        boolean z = false;
        boolean z2 = colRef.length() > 0 && colRef.charAt(0) == '$';
        this._isColAbs = z2;
        colRef = z2 ? colRef.substring(1) : colRef;
        if (colRef.length() == 0) {
            this._colIndex = -1;
        } else {
        ...
        if (rowRef.length() > 0 && rowRef.charAt(0) == '$') {
            z = true;
        }
        this._isRowAbs = z;
        rowRef = z ? rowRef.substring(1) : rowRef;
        if (rowRef.length() == 0) {
            this._rowIndex = -1;
        } else {
    ...
    public boolean equals(Object o) {
        ...
        if (this._rowIndex == cr._rowIndex && this._colIndex == cr._colIndex && this._isRowAbs == cr._isRowAbs && this._isColAbs == cr._isColAbs) {
            ...
            } else if (str.equals(cr._sheetName)) {
                return true;
            }
        }
        return false;
    ...
    public int hashCode() {
        int i = ((((((((17 * 31) + this._rowIndex) * 31) + this._colIndex) * 31) + (this._isRowAbs ? 1 : 0)) * 31) + (this._isColAbs ? 1 : 0)) * 31;
        String str = this._sheetName;
