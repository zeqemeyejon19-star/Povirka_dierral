...
public final class Detector {
    ...
    public static PDF417DetectorResult detect(BinaryBitmap image, Map<DecodeHintType, ?> hints, boolean multiple) throws NotFoundException {
        ...
        if (listDetect.isEmpty()) {
            BitMatrix bitMatrixM7clone = bitMatrix.m7clone();
            bitMatrix = bitMatrixM7clone;
            bitMatrixM7clone.rotate180();
    ...
    private static ResultPoint[] findRowsWithPattern(BitMatrix matrix, int height, int width, int startRow, int startColumn, int[] pattern) {
        ...
        while (true) {
            ...
            if (iArrFindGuardPattern != null) {
                ...
                while (true) {
                    ...
                    startRow2--;
                    int[] iArrFindGuardPattern2 = findGuardPattern(matrix, startColumn, startRow2, width, false, pattern, counters);
                    previousRowLoc = iArrFindGuardPattern2;
        ...
        if (found) {
            ...
            while (true) {
                ...
                skippedRowCount = skippedRowCount2;
                stopRow = stopRow3;
                int[] iArrFindGuardPattern3 = findGuardPattern(matrix, previousRowLoc2[0], stopRow3, width, false, pattern, counters);
                loc2 = iArrFindGuardPattern3;
                if (iArrFindGuardPattern3 != null && Math.abs(previousRowLoc2[0] - loc2[0]) < 5 && Math.abs(previousRowLoc2[1] - loc2[1]) < 5) {
